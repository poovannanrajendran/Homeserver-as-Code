...
flowchart LR
  ...
  Tailnet --> PVE["Proxmox Host<br/>(single node)"]
  ...
  VM100 --> CPLAT["Platform<br/>n8n/Jenkins/Nginx"]
  VM100 --> COBS["Observability<br/>Prometheus/Grafana"]
...
flowchart TB
  subgraph LAN["Home LAN (example docs range: 192.0.2.0/24)"]
    GW["Gateway/DNS<br/>192.0.2.1"]
    PVE["Proxmox<br/>192.0.2.250"]
    DKR["docker-host-01<br/>192.0.2.20"]
    AI["ai-node-01<br/>192.0.2.24"]
    RUN["automation-runner-01<br/>192.0.2.30"]
    MED["media-01<br/>192.0.2.26"]
    PBS["pbs-01<br/>192.0.2.25"]
  end

  subgraph TS["Tailscale Tailnet"]
    T1["docker-host-01<br/>100.x"]
    T2["ai-node-01<br/>100.x"]
    T3["automation-runner-01<br/>100.x"]
    T4["media-01<br/>100.x"]
    T5["pbs-01<br/>100.x"]
  end
...
```mermaid
flowchart LR
  NVME["Host NVMe<br/>VM disks + primary runtime"] --> PVE["Proxmox Host"]
  USB["External 4TB<br/>/mnt/usb-4tb"] --> PVE

  PVE --> NFS1["/mnt/usb-4tb/nextcloud-data"]
  PVE --> NFS2["/mnt/usb-4tb/media"]
  PVE --> NFS3["/mnt/usb-4tb/pbs-datastore"]

  NFS1 --> DKR["docker-host-01<br/>/mnt/nextcloud-data"]
  NFS2 --> DKR2["docker-host-01<br/>/mnt/media-library"]
